...
```
(root user or sudo) ./MassVulScan.sh --auto-installation-latest (latest packages of Nmap and Masscan with compilation ~5 minutes)
(root user or sudo) ./MassVulScan.sh --auto-installation-apt (APT installation, speedest, without the latest versions of Nmap and Masscan ~1 minute)
```
