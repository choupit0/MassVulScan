...
usage(){
        ...
        echo -e ${_color}"              Input file including IPv4 addresses (no hostname), compatible with subnet mask."${end_color}
        echo "          Example:"
...
nb_hosts_masscan="$(grep -v ^"#" ${hosts} | wc -l)"
...
nb_ports="$(grep ^open masscan-output.txt | wc -l)"
...
vuln_hosts_count="$(for i in ${nmap_temp}/*.nmap; do tac $i | sed -n -e '/|_.*CVE-\|VULNERABLE/,/^Nmap/p' | tac ; done | grep -c "Nmap")"

if [[ ${vuln_hosts_count} != "0" ]]; then
	vuln_hosts="$(for i in ${nmap_temp}/*.nmap; do tac $i | sed -n -e '/|_.*CVE-\|VULNERABLE/,/^Nmap/p' | tac ; done)"
	vuln_hosts_ip="$(for i in ${nmap_temp}/*.nmap; do tac $i | sed -n -e '/|_.*CVE-\|VULNERABLE/,/^Nmap/p' | tac ; done | grep ^"Nmap scan report for" | cut -d" " -f5)"
	...
	vuln_hosts_format="$(awk '{print $1 "\t" $NF}' vulnerable_hosts.txt |  sed 's/3(NXDOMAIN)/\No reverse DNS entry found/' | sort -t . -n -k1,1 -k2,2 -k3,3 -k4,4)"
...
echo "<nmaprun args=\"nmap --max-retries 2 --max-rtt-timeout 500ms -p[port(s)] -Pn -sT -sV -n --script vulners [ip(s)]\" scanner=\"Nmap\" start=\"\" version=\"${nmap_version}\" xmloutputversion=\"1.04\">" >> nmap-output.xml
